...
VouchDAO 是一个由社区主导的、建立在 Vouch 标准之上的去中心化验证层。开发者建立背书服务，VouchDAO 社区的成员投票决定哪些验证服务值得信赖。

一旦建立了新的服务，其地址将在 [VouchDAO community.xyz](https://community.xyz/#_z0ch80z_daDUFqC9jHjfOL8nekJcok4ZRkE_UesYsk) 界面上进行投票。如果投票通过，它将被新增为经过验证的背书。

<img src="https://arweave.net/7W9krszlEXdR38LB7uXgJ_EPXGj-woXljsA5h5GpGzk" />

## 工作原理

开发者可以建立不同的 Vouch 服务，根据给定的一组要求对用户的 Arweave 钱包进行验证。目前的一个例子是 Twitter 服务，这是第一个背书服务，迄今已为超过 180 个 Arweave 地址提供了背书。

VouchDAO 智能合约状态具有一个 `vouched` 属性。每当用户进行验证时，状态会更新。`vouched` 对象以以下格式存储一系列被背书的地址：
...
| ------------------- | ---------- | ------------------------------------------------------------------- |
| App-Name            | 否         | `Vouch`                                                             |
| Vouch-For           | 否         | 在此交易中为其提供背书的 Arweave `address`                          |
| App-Version         | 是         | `0.1`                                                               |
| Verification-Method | 是         | 验证此人身份的方法。例如 - `Twitter`/`In-Person`/`Gmail`/`Facebook` |
